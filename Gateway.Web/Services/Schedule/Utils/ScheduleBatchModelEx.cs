    ...
    public static class ScheduleBatchModelEx
    {
        public static ScheduleBatchModel ToBatchInputModel(this Database.Schedule schedule)
        {
            return new ScheduleBatchModel
            {
                ...
                TradeSources = schedule.TradeSource == schedule.Site ? schedule.TradeSource : $"{schedule.TradeSource};{schedule.Site}",
        ...
        public static ScheduleBatchModel ToBatchInputModel(this IList<Database.Schedule> schedules)
        {
            return new ScheduleBatchModel
            {
                ...
                TradeSources = string.Join(",\n",schedules.Select(x => x.TradeSource == x.Site ? x.TradeSource : $"{x.TradeSource};{x.Site}").Distinct().ToList()),
                Group = string.Empty,
                Parent = string.Empty,
        ...
        public static void SetConfigurationTypes(this ScheduleBatchModel model, IBatchConfigDataService service, string[] selectedValues)
        {
            model.Types = service
                .GetConfigurations()
                .Select(x => new SelectListItem { Value = x.ConfigurationId.ToString(), Text = $"{x.Type} ({x.TradeSourceType})" })
                .ToList();
